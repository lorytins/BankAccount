...
func DepositHandler(w http.ResponseWriter, r *http.Request) {
	...
	request := new(requests.DepositRequest)
	...
	if err == nil {
		operation := models.NewDepositOperation(account.Id, utils.ConvertToCents(request.Amount))

		messageErr := account.Deposit(operation)
		if messageErr == "" {
			w.WriteHeader(http.StatusCreated)
			json.NewEncoder(w).Encode("Deposit made successfully")
		} else {
			w.WriteHeader(http.StatusBadRequest)
